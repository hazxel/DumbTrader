...
class OkxWsListenTradesAllClient(OkxWsClient):
    async def subscribe(self, inst_id):
        subscribe_msg = build_subscribe_trades_all_msg(OKX_WS_SUBSCRIBE_CHANNEL.TRADES_ALL, inst_id)
        connection_id = await super().subscribe(subscribe_msg)
        return connection_id

class OkxWsListenOrdersClient(OkxWsPrivateClient):
    async def subscribe(self, inst_type, inst_id):
        subscribe_msg = build_subscribe_orders_msg(OKX_WS_SUBSCRIBE_CHANNEL.ORDERS, inst_type, inst_id)
        connection_id = await super().subscribe(subscribe_msg)
        return connection_id
