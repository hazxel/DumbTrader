...
class OkxWsChannel:
    def __init__(self, websocket):
        ...
        self.conn_id = None
        # not saving msg id generated by client
        # not saving order id generated by exchange
        ...
        while True:
            try:
                ...
                try:
                    ...
                    print("unable to get response, aboarting...")
                    return e
            return response
...
class OkxWsPrivateChannel(OkxWsChannel):
    ...
    async def submit_order(self, client_order_id, inst_id, td_mode, side, pos_side, order_type, sz, px):
        ...
        # return data["data"][0]["ordId"] # not using order id generated by exchange
...
def build_login_msg(api_key, passphrase, secret_key):
    ...
    return build_json_msg("login", login_args)

def build_subscribe_msg(channel, inst_id):
    subscribe_args = [{"channel": channel,"instId": inst_id}]
    return build_json_msg("subscribe", subscribe_args)
...
def build_submit_order_msg(client_order_id, inst_id, td_mode, side, pos_side, order_type, sz, px):
    order_args = [{"instId": inst_id,
                   ...
                   "sz": sz,
                   "px": px}]
    return build_json_msg("order", order_args, str(uuid.uuid4()).replace('-', ''))

def build_cancel_order_msg(inst_id, client_order_id):
    order_args = [{"instId": inst_id, "clOrdId": client_order_id}]
    return build_json_msg("cancel-order", order_args, str(uuid.uuid4()).replace('-', ''))
